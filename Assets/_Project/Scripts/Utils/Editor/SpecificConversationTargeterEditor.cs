...
namespace FunForLab
{
    [CustomEditor(typeof(SpecificConversationTargeter), true)]
    public class SpecificConversationTargeterEditor : Editor
    {
        private SpecificConversationTargeter sct;
        private DialogueEntryPicker entryPicker = null;

        public override void OnInspectorGUI()
        {
            sct = target as SpecificConversationTargeter;
            if (sct == null) return;
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        entryIDProperty.intValue = Mathf.Max(0,
                            entryPicker.DoLayout("Entry ID", entryIDProperty.intValue));
                    }
                    else
                    {
                        entryIDProperty.intValue = Mathf.Max(0,
                            EditorGUILayout.IntField(new GUIContent("Entry ID", "Start at this entry ID."),
                                entryIDProperty.intValue));
                    }
                }
            }
            finally
